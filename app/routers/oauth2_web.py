...
async def oauth2_dashboard(
    ...
    available_services = (
        list(get_oauth2_service_config("github").keys())
        if get_oauth2_service_config("github")
        else []
    )
    available_scopes = get_available_scopes()

    return templates.TemplateResponse(
        "oauth2_dashboard.html",
        {
            "request": request,
            "user": current_user,
            "tokens": user_tokens,
...
async def create_token(
    ...
    if not validate_scope(token_data.scope, token_data.service_name):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                f"Invalid scope '{token_data.scope}' for service "
    ...
    # Store token
    try:
        stored_token = token_manager.store_token(
            db, current_user["id"], token_data, request
        )
        return stored_token
    except Exception as e:
        raise HTTPException(
...
async def get_decrypted_token(
    ...
    token = token_manager.get_decrypted_token(
